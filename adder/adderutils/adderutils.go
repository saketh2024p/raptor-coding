...
import (
	...
	"github.com/ipfs-cluster/ipfs-cluster/adder/raptor" // Example Raptor-specific package
...
// allows customizing the HTTP response output format to something
...
func AddMultipartHTTPHandler(
	...
	// (no keep-alive) or things break badly when adding.
	// https://github.com/ipfs/go-ipfs-cmds/pull/116
	w.Header().Set("Connection", "close")

	var wg sync.WaitGroup
	if !params.StreamChannels {
		// Buffer responses in memory and return them as a valid JSON array.
		wg.Add(1)
		...
		// Handle erasure coding (Raptor-based or legacy)
		if params.Erasure {
			dgs2 := raptor.New(ctx, rpc, params) // Initialize Raptor-specific DAG service
...
// streamOutput streams data to the client.
...
// buildOutput collects data from the output channel into a slice.
